...
The reference profile `prospero-cex-12.40` now carries `software-version = { display = "13.090.001",
packed = 0x13090001 }` alongside `firmware = 0x1240` - the two-numbers-one-console fact made data.
End-to-end verified: obSCEne's 130-layout dump under `--profile prospero-cex-12.40` emits 13.090.001 /
...
`["prospero-cex-12.40".software-version]`, not `[prospero-cex-12.40.software-version]`, or the dots parse as
