...
fn bytes_of(text: &str, symbol: &str) -> Result<Vec<u8>> {
    ...
    for (offset, bytes) in rows {
        anyhow::ensure!(
            offset == out.len(),
            "{symbol}: rows jump from {} to {offset} - the capture is missing a row, and every \
...
fn verify(symbol: &str, entries: &[u16], checks: &[SpotCheck]) -> Result<()> {
    ...
    for check in checks {
        ...
        anyhow::ensure!(
            got == check.value,
            "{symbol}: {} says {:#x}, but the table's entry {} is {got:#x} - the parse is wrong, \
...
fn header(capture: &str) -> String {
    ...
    format!(
        "# Character-classification tables, measured on real hardware.\n\
         ...
         #\n\
         # Source: obSCEne probe `{PROBE}`, capture `{capture}`.\n\
         #\n\
         ...
         #\n\
         # Entries run from index {first} upward, because C indexes these with a character\n\
...
mod tests {
    ...
    fn a_gap_in_the_rows_is_refused() {
        let text = "OBS|bytes|035-libc/getpctype|_Getpctype|table_raw_neg16|0|0100\n\
