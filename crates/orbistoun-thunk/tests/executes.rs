//! Executes real thunks.
//!
//! The unit tests assert the encoding; this asserts that it runs, because an instruction wrong
//! by one bit assembles into a plausible different instruction. It is its own binary because the
//! recording statics are process-global (a thunk has no register left for a context pointer), so
//! call-order assertions hold only in a process that made exactly these calls.
...
/// Executed thunks record their own index in call order and hand the guest the answer.
#[test]
fn thunks_execute_record_their_own_index_and_answer_the_guest() {
    ...
    // A stub past the end must not resolve to another import's, which would give a wrong trace.
    assert!(table.address_of(16).is_none(), "out of range must be None");
    assert_eq!(table.len(), 16);
    assert!(!table.is_empty());

    // Guest code enters a stub knowing nothing and comes back with an answer, while the table
    // records which import it wanted.
    ...
    // Order makes a boot trace readable; a histogram alone loses it.
