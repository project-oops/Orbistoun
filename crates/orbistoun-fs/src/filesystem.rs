//! The guest filesystem: a base tree this project can account for, and a title's own files
//! layered over it.
//!
//! Every path in the base is a claim about the platform, and git does not track empty
//! directories, so the source of truth is [`MANIFEST`], a knowledge file, and the directories
//! are materialised from it. They can be deleted and rebuilt at any time (D251).
...
/// One directory the platform is known to have.
...
/// Reads the manifest.
///
/// A small hand-rolled reader rather than a TOML dependency: the file is a flat list, and
...
///
/// Idempotent, and it never deletes: an existing directory is left alone, and one added by
/// hand survives.
...
mod tests {
    ...
    ///
    /// A reader that produced nothing would hand every guest an empty filesystem with no
    /// error (D251).
    #[test]
    fn the_shipped_manifest_describes_the_paths_it_claims() {
        ...
        // Every entry says how it is known.
        for entry in &found {
            assert!(
    ...
    ///
    /// A path orbistoun invented would give a guest a fabricated platform fact.
    #[test]
