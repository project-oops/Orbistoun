//! Starting another title: `sceSystemServiceLaunchApp` (worklog 842).
//!
//! A launcher lists the installed titles and asks the system to start one. On the device the
//! system suspends the caller and brings the named title up in its place; here one process runs one
//! guest, so the request is handed to whoever owns the run - the worker streams it to the front end,
//! which ends this run and starts the title. The guest is told the request was taken, as it would
//! be, and carries on until its run is ended.
...
///
/// The console's own error for this is unmeasured; this is the reserved placeholder range's
/// invalid-argument code (D670), so it cannot be read as a real one.
...
fn title_id(address: u64) -> Option<String> {
    ...
    for offset in 0..MAX_TITLE_ID {
        ...
        // time so the scan stops at the terminator rather than straddling past it.
