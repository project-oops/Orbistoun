...
static LINK_PLAN: std::sync::OnceLock<orbistoun_proto::LinkSummary> = std::sync::OnceLock::new();
...
pub fn note_link_plan(summary: orbistoun_proto::LinkSummary) {
    let _ = LINK_PLAN.set(summary);
}
...
fn linked_conditions() -> Conditions {
    ...
    if let Some(summary) = LINK_PLAN.get() {
        conditions.link_plan.clone_from(&summary.digest);
        conditions.link_plan_stored.clone_from(&summary.stored);
        conditions.link_plan_differs.clone_from(&summary.differs);
        conditions.link_syscalls = summary.syscalls;
    }
    conditions
}
